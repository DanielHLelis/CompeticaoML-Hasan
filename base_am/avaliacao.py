        ...
        for i, fold in enumerate(self.folds):
            ...
            if(self.ClasseObjetivoOtimizacao is not None):
                study = optuna.create_study(
                    sampler=self.sampler, direction="maximize")
                objetivo_otimizacao = self.ClasseObjetivoOtimizacao(fold)
                # n_jobs=-1 parallelizes the operation
                study.optimize(objetivo_otimizacao, self.num_trials, n_jobs=-1)
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        for fold_validacao in self.fold.arr_folds_validacao:
            resultado = metodo.eval(
                fold_validacao.df_treino, fold_validacao.df_data_to_predict, self.fold.col_classe)
